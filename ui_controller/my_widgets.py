...
class clickable_label(QLabel):
    ...
    def mousePressEvent(self, e):
        ...
        if e.buttons() == Qt.LeftButton:
            self.event.label_press(point, "L")
        if e.buttons() == Qt.RightButton:
            self.event.label_press(point, "R")

    def mouseMoveEvent(self, e):
        point = (e.pos().x(), e.pos().y())
        self.event.label_move(point)

    def mouseReleaseEvent(self, e):
        point = (e.pos().x(), e.pos().y())
        self.event.label_release(point)  
...
class my_listV(QListView):
    def __init__(self, parent):
        QListView.__init__(self, parent)
        self.setStyleSheet('QListView {background-color: #000000; color: #E6E6E6;}')
...
class my_cb(QCheckBox):
    def __init__(self, parent):
        ...
        self.setStyleSheet('QCheckBox {background-color: #1E1E1E; color: #E6E6E6;}')
